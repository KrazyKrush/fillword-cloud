...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await startSolve(userId, parseInt(req.params.fillwordId as string));
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await checkWord(
        parseInt(req.params.resultId as string),
        data.word,
        data.cells as Array<{ row: number; col: number }>
      );
      res.json(result);
    } catch (error: any) {
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const result = await getLeaderboard(parseInt(req.params.fillwordId as string));
      res.json(result);
    } catch (error: any) {
